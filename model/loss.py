...
class DLINEXLoss(nn.Module):
    ...
    def forward(self, outputs, targets, categories):
        ...
        for i in range(self.category):
            domain_mask = (categories == i).nonzero(as_tuple=False).squeeze()
            ...
            if domain_targets.dim() == 2:
                ...
                true_class_indices = domain_targets.argmax(dim=0)
                true_class_indices = true_class_indices.unsqueeze(0) 

            sign_matrix = self.sign_matrices[i]

            true_class_signs = sign_matrix[true_class_indices]

            true_class_probs = domain_outputs.gather(1, true_class_indices.unsqueeze(1)).squeeze()
