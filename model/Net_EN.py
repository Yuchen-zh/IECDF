...
class MDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, out_channels, dropout, category_dict, domain_num=9):
        ...
        self.ClipModel, preprocess = clip.load('/HOME/pxyai/pxyaih_0031/Performance01/IECDF/ViT-B-16.pt', device="cuda")
...
class Trainer():
    ...
    def train(self):
        ...
        for epoch in range(self.epoches):
            ...
            results0, _, _, _ = self.test(self.val_loader)
    ...
    def test(self, dataloader):
        ...
        return metricsTrueFalse(label1, pred0, category, self.category_dict, save_dir=self.save_param_dir, domain_num=self.domain_num),metricsTrueFalse(label1, pred1, category, self.category_dict, save_dir=self.save_param_dir, domain_num=self.domain_num),metricsTrueFalse(label1, pred2, category, self.category_dict, save_dir=self.save_param_dir, domain_num=self.domain_num), metricsTrueFalse(label1, pred3, category, self.category_dict, save_dir=self.save_param_dir, domain_num=self.domain_num)
